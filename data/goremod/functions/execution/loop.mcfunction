...
execute as @e[tag=gm_tracked] at @s unless entity @p[distance=..16] anchored eyes positioned ^ ^ ^ run function goremod:execution/clean_tracking

# execute
execute as @e[type=area_effect_cloud,tag=gm_tracking_init,nbt={Age:2}] at @s run function goremod:execution/execute
