...
execute if score UltraBlood gm.options matches 1 if score Blood gm.options matches 1 if entity @p[predicate=gm:blood/dmg_l,predicate=gm:blood/dmg_dealer,distance=..8] as @e[predicate=gm:execution/aec_tracking,sort=nearest,limit=1,distance=..3] at @s run function gm:execution/execute/execute
